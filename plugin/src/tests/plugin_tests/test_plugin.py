...
@pytest.mark.parametrize(
    "url_template",
    [
        pytest.param("{url}", id="plain"),
        pytest.param("{url}/", id="trailing-slash"),
        pytest.param("  {url}  ", id="whitespace-padded"),
    ],
)
def test_client_url_from_config(mocker: MockerFixture, fake_server_url: str, url_template: str) -> None:
    ...
    beets.config["beetkeeper_plugin"]["server_url"].set(url_template.format(url=fake_server_url))
    BeetkeeperPlugin()
