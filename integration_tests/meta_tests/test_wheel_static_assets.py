...
The wheel is built with `generate_setup=False` (see `beetkeeper-core/BUILD`), so *setuptools* — not Pants —
decides which non-`.py` files land in the wheel, driven entirely by `[tool.setuptools.package-data]` in
`beetkeeper-core/pyproject.toml`. The static assets under `beetkeeper/api/static/**` live in non-package subdirs
...
_PACKAGE_RELPATH: Final[str] = "beetkeeper-core/src/beetkeeper"
_STATIC_RELPATH: Final[str] = "beetkeeper-core/src/beetkeeper/api/static"
...
def test_all_static_assets_are_covered_by_package_data(
    ...
    assert not uncovered, (
        ...
        "(e.g. `api/static/**/*.<ext>`) to `beetkeeper-core/pyproject.toml`."
    )
