...
from beetsplug._utils.requests import RequestHandler, TimeoutAndRetrySession
...
class BeetkeeperListener(BeetsPlugin):
    """
    ...
    def __init__(self, beetkeeper_server_url: str, raw_api_token: str):
        self._client = _BeetKeeperClient(url=beetkeeper_server_url, api_token=_APIToken(value=raw_api_token))
...
class _BeetKeeperClient(RequestHandler):
    ...
    def post(self, event_type: EventType, event_element: Album | Item | ImportTask | ImportSession) -> Response:
        ...
        return self.request(
            method="post",
            url=self._base_path + self._url_subpath,
...
def _jsonify(event_type: EventType, event_element: Album | Item | ImportTask | ImportSession) -> dict[str, Any]:
    ...
    if isinstance(event_element, (Album, Item)):
        ...
        if imported_items := event_element.imported_items():
            jsonified_imported_items = [
                _jsonify(event_type=event_type, event_element=item) for item in imported_items
            ]
        return dict(
            event_type=event_type,
            choice_flag=event_element.choice_flag,
            imported_items=jsonified_imported_items,
        )
