...
2. **Autogenerate a revision.** Point alembic at a throwaway SQLite DB via `-x db_url=...` and let it
   ...
   ```bash
   alembic -c beetkeeper-core/src/beetkeeper/db/alembic.ini \
...
5. **Run the sync tests** (these fail if the models and migrations have drifted):
   ...
   ```bash
   uv run --all-groups pytest ./beetkeeper-core/src/tests/db_tests
   # or the full gate: pants test ::
   ```

   See `beetkeeper-core/src/tests/db_tests/test_alembic_and_models_synced.py` — `test_no_pending_autogenerate_diff`
